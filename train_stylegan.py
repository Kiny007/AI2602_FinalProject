...
训练逻辑与 `train.py` 共享一套运行时能力：多卡 DDP、子进程管理、EMA
生成器、统一日志/可视化与 checkpoint 保存；模型和损失则保留为本脚本特有。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    default_config = PROJECT_ROOT / "src" / "gan_faces" / "config" / "stylegan_lite.yaml"
    ...
    parser.add_argument("--batch-size", type=int, default=64, help="总 batch size，多卡时会自动按卡均分")
            ...
            for step, real_images in enumerate(dataloader, start=1):
                ...
                metric_tensor = torch.stack(
                    [
                        loss_d.detach(),
                        loss_g.detach(),
                        torch.sigmoid(real_logits.detach()).mean(),
                        torch.sigmoid(fake_logits.detach()).mean(),
                    ]
                )
                ...
                if is_rank0:
                    ...
                    if should_save:
                        torch.save(state, layout.checkpoint_dir / f"stylegan_lite_nimg_{cur_nimg:08d}.pt")
